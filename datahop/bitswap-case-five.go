...
func BitswapTestCase5(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	...
	tag := "tag"
	if seq == 1 {
		err := addFile(ctx, runenv, tag, comm, hundredM)
		if err != nil {
			log.Fatal(err)
		}
	}
...
func BitswapTestCase5Ten(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5Hundred(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5Rand(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
...
func BitswapTestCase5ZeroOne(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5TenOne(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5HundredOne(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5RandOne(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	mRand.Seed(time.Now().UnixNano())
	r := mRand.Intn(max-ten+1) + ten
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
...
func BitswapTestCase5ZeroTen(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5TenTen(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5HundredTen(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5RandTen(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	mRand.Seed(time.Now().UnixNano())
	r := mRand.Intn(max-ten+1) + ten
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
...
func BitswapTestCase5ZeroHund(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5TenHund(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5HundredHund(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
	defer cancel()
	initCtx.MustWaitAllInstancesInitialized(ctx)
...
func BitswapTestCase5RandHund(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	mRand.Seed(time.Now().UnixNano())
	r := mRand.Intn(max-ten+1) + ten
	totalTime := time.Minute * 40
	ctx, cancel := context.WithTimeout(context.Background(), totalTime)
